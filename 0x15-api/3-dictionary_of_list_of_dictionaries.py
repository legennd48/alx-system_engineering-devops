...
def fetch_all_todo_data():
    ...
    users_data = requests.get("http://jsonplaceholder.typicode.com/users").json()
    todos_data = requests.get("http://jsonplaceholder.typicode.com/todos").json()

    users = [(user.get('id'), user.get('username')) for user in users_data]
    task_status_title = [(todo.get('userId'), todo.get('completed'), todo.get('title')) for todo in todos_data]
...
def export_to_json(users_data, todos_data):
    ...
    for user in users_data:
        ...
        for task in todos_data:
            if task[0] == user[0]:
                tasks.append({"task": task[2], "completed": task[1], "username": user[1]})
